...
const receitaExibida = computed(() =>
  visao.value === 'caixa' ? resumo.value.receitaRecebida : resumo.value.receita,
)
...
const alertaFinanceiro = computed(() => {
  ...
  if (vencidas > 0)
    return `${vencidas} conta${vencidas > 1 ? 's' : ''} vencida${vencidas > 1 ? 's' : ''} — ver Contas`
...
function adicionarConta(): void {
  ...
  try {
    fin.adicionarConta({
      descricao: novaContaDesc.value,
      valor: novaContaValor.value ?? 0,
      vencimento: novaContaVenc.value,
      tipo: novaContaTipo.value,
    })
    novaContaDesc.value = ''
...
function adicionarCustoFixo(): void {
  ...
  try {
    fin.adicionarCustoFixo(
      {
        descricao: cfDesc.value,
        categoria: cfCategoria.value,
        centroCusto: cfCentro.value,
        valorMensal: cfValor.value ?? 0,
      },
...
const dreRealTurno = computed(() =>
  fin.dreReal(
    mercado.resumoOperacional.receita,
    mercado.resumoOperacional.cmv,
    mercado.resumoOperacional.desperdicio,
  ),
)
...
const barrasAging = computed<BarraDoGrafico[]>(() =>
  FIADO_AGING_MOCK.map((f) => ({
    label: f.faixa,
    valor: f.valor,
    valorLabel: formatarMoedaCompacto(f.valor),
  })),
)
...
function exportarRelatorios(): void {
  const linhas = [
    ...
    ...relOperador.value.map(
      (o) =>
        `operador;${o.operador};${o.vendas};${o.receita};${o.ticketMedio ?? ''};${o.desconto};${o.cancelamentos}`,
    ),
    'pagamento;vendas;receita;participacao',
    ...relPagamento.value.map(
      (p) => `pagamento;${p.forma};${p.vendas};${p.receita};${(p.participacao * 100).toFixed(1)}%`,
    ),
    'fornecedor;receita;cmv;lucro;margem',
    ...relFornecedor.value.map(
      (f) => `fornecedor;${f.fornecedor};${f.receita};${f.cmv};${f.lucro};${f.margem ?? ''}`,
    ),
    'margem-real;produto;grupo;subgrupo;receita;cmv;lucro;abc',
    ...relMargemReal.value.map(
      (m) => `margem;${m.produto};${m.grupo};${m.subgrupo};${m.receita};${m.cmv};${m.lucro};${m.classeABC}`,
    ),
  ]
  const blob = new Blob([linhas.join('\n')], { type: 'text/csv' })
  const url = URL.createObjectURL(blob)
  ...
  <div class="pagina">
    ...
    <template v-else>
      <div class="abas">
        ...
        <button type="button" :class="{ ativa: aba === 'itens' }" @click="aba = 'itens'">
          Lucro por item
        </button>
        ...
        <button type="button" :class="{ ativa: aba === 'detalhe' }" @click="aba = 'detalhe'">
          Detalhamento
        </button>
        <button type="button" :class="{ ativa: aba === 'relatorios' }" @click="aba = 'relatorios'">
          Relatórios
        </button>
      </div>
      ...
      <template v-if="aba === 'resumo'">
        <div class="cartao-secao destaque-real">
          ...
          <div class="cartoes">
            <div class="cartao">
              <span>Receita real</span><strong>{{ formatarMoeda(mercado.resumoOperacional.receita) }}</strong>
            </div>
            <div class="cartao">
              <span>CMV real</span><strong>{{ formatarMoeda(mercado.resumoOperacional.cmv) }}</strong>
            </div>
            <div class="cartao">
              <span>Margem real</span><strong>{{ formatarMoeda(mercado.resumoOperacional.margem) }}</strong>
            </div>
            <div class="cartao">
              <span>Perdas sessão</span
              ><strong>{{ formatarMoeda(mercado.resumoOperacional.desperdicio) }}</strong>
            </div>
          </div>
          <div class="legenda">
            DRE real do turno: lucro {{ formatarMoeda(dreRealTurno.lucroLiquido) }} · fixos
            {{ formatarMoeda(dreRealTurno.custosFixos) }} · contas pagas
            {{ formatarMoeda(dreRealTurno.contasPagas) }}
          </div>
          <div v-if="mercado.vendas.length === 0" class="legenda">
            Sem vendas no turno —
            <button type="button" class="link" @click="carregarDemoFin">carregar demonstração</button> para
            visualizar.
          </div>
        </div>
        <div class="cartao-secao">
          <div class="secao-titulo">
            Custos fixos mensais — {{ formatarMoeda(fin.totalCustosFixosAtivos) }}/mês
          </div>
          <div class="grade-params">
            <label class="campo"
              >Descrição<input v-model="cfDesc" type="text" placeholder="ex.: Aluguel"
            /></label>
            <label class="campo">Categoria<input v-model="cfCategoria" type="text" /></label>
            <label class="campo"
              >Valor/mês (R$)<input v-model.number="cfValor" type="number" min="0" step="0.01"
            /></label>
          </div>
          <div v-if="erroCf" class="erro">{{ erroCf }}</div>
          <button type="button" class="botao-primario" :disabled="!podeLancar" @click="adicionarCustoFixo">
            Adicionar custo fixo
          </button>
          <div v-if="fin.custosFixos.length === 0" class="legenda">
            Nenhum custo fixo cadastrado — cadastre aluguel, folha, energia ou
            <button type="button" class="link" @click="fin.carregarExemplo(auth.usuario?.nome ?? 'demo')">
              usar exemplo</button
            >.
          </div>
          <div v-for="c in fin.custosFixos" :key="c.id" class="linha-custo">
            ...
            <span class="acoes-conta"
              ><strong>{{ formatarMoeda(c.valorMensal) }}/mês</strong>
              <button type="button" class="link" @click="fin.alternarCustoFixo(c.id)">
        ...
        <div class="cartoes">
          ...
          <div class="cartao">
            ...
            <strong>{{
              resumo.margemLiquida === null ? 'sem base' : formatarPercentual(resumo.margemLiquida)
            }}</strong>
          </div>
          <div class="cartao">
            <span>Fiado em aberto</span>
        ...
        <div class="cartao-secao">
          ...
          <GraficoBarras :dados="barrasFluxo" />
          <div class="legenda">
            Entradas {{ formatarMoeda(resumo.receita) }} · Saídas
            {{ formatarMoeda(resumo.cmv + resumo.despesas) }} · Desperdício
            {{ formatarMoeda(resumo.desperdicio) }} · Saldo {{ formatarMoeda(resumo.lucroLiquido) }}
          </div>
        </div>
        ...
        <div class="cartao-secao">
          ...
          <div class="grade-params">
            <label class="campo"
              >Margem alvo (0–0,9)<input
                v-model.number="margemAlvo"
                type="number"
                min="0"
                max="0.9"
                step="0.01"
            /></label>
            <label class="campo"
              >Custos fixos/mês (R$)<input v-model.number="custosFixos" type="number" min="0" step="100"
            /></label>
            <label class="campo"
              >Índice margem contribuição<input
                v-model.number="indiceMC"
                type="number"
                min="0"
                max="1"
                step="0.01"
          ...
          <div class="legenda">
            Ponto de equilíbrio: <strong>{{ pe === null ? 'sem base' : formatarMoeda(pe) }}</strong
            >/mês
          </div>
        </div>
      </template>
      ...
      <template v-if="aba === 'desperdicios'">
        <div class="cartoes">
          <div class="cartao">
            <span>Perda total</span><strong>{{ formatarMoeda(resumoDesp.totalReais) }}</strong>
          </div>
          <div class="cartao">
            <span>Peso perdido</span><strong>{{ resumoDesp.totalKg.toFixed(1) }} kg</strong>
          </div>
          <div class="cartao">
            <span>% da receita</span>
            <strong>{{
              resumoDesp.percentualReceita === null
                ? 'sem base'
                : formatarPercentual(resumoDesp.percentualReceita)
            }}</strong>
          </div>
        </div>
        ...
        <div class="cartao-secao">
          ...
          <div class="grade-params">
            <label class="campo"
              >Produto<input v-model="novoDespProduto" type="text" placeholder="ex.: Picanha"
            /></label>
            <label class="campo"
              >Qtd (kg)<input v-model.number="novoDespQtd" type="number" min="0" step="0.001"
            /></label>
            <label class="campo"
              >Custo/kg (R$)<input v-model.number="novoDespCusto" type="number" min="0" step="0.01"
            /></label>
            <label class="campo"
              >Motivo
              <select v-model="novoDespMotivo">
                <option v-for="(rotulo, valor) in ROTULO_MOTIVO_DESPERDICIO" :key="valor" :value="valor">
                  {{ rotulo }}
                </option>
              </select>
            </label>
          ...
          <button type="button" class="botao-primario" :disabled="!podeLancar" @click="adicionarDesperdicio">
            Registrar perda
          </button>
        </div>
        ...
        <div class="cartao-secao">
          ...
          <div v-for="item in resumoDesp.porMotivo" :key="item.motivo" class="linha-custo">
            <span
              >{{
                ROTULO_MOTIVO_DESPERDICIO[item.motivo as keyof typeof ROTULO_MOTIVO_DESPERDICIO] ??
                item.motivo
              }}
              · {{ item.quantidade.toFixed(1) }} kg</span
            >
      ...
      <template v-if="aba === 'itens'">
        <div v-if="!podeVerMargem" class="cartao-secao">
          ...
          <div class="legenda">
            Margem por item exige a permissão <strong>financeiro.ver_margem</strong> (Proprietário).
          </div>
        </div>
        <template v-else>
          <div class="cartoes">
            <div class="cartao">
              ...
              ><strong>{{ mediaMargem === null ? 'sem base' : formatarPercentual(mediaMargem) }}</strong>
            </div>
            <div class="cartao">
              <span>Itens em risco</span
              ><strong>{{ MARGEM_POR_ITEM_MOCK.filter((i) => i.emRisco).length }}</strong>
            </div>
          </div>
          <div class="cartao-secao">
            ...
            <table class="tabela">
              ...
              <tbody>
                <tr v-for="item in abc" :key="item.produtoId" :class="{ risco: item.emRisco }">
                  <td>
                    {{ item.produto }}<br /><small
                      >{{ item.quantidade }} {{ item.unidade }} · custo
                      {{ formatarMoeda(item.custoMedio) }}</small
                    >
                  </td>
                  <td>{{ formatarMoeda(item.receita) }}</td>
                  <td>{{ formatarMoeda(item.cmv) }}</td>
                  <td>
                    {{ formatarMoeda(item.margem) }}<br /><small>{{
                      item.margemPercentual === null ? '—' : formatarPercentual(item.margemPercentual)
                    }}</small>
                  </td>
            ...
            <div class="legenda">
              Custo congelado na venda (compra + desossa + maturação). Linha destacada = margem em risco.
            </div>
          </div>
        </template>
      </template>
      ...
      <template v-if="aba === 'contas'">
        <div class="cartao-secao">
          ...
          <div class="grade-params">
            <label class="campo"
              >Descrição<input v-model="novaContaDesc" type="text" placeholder="ex.: Fornecedor carne"
            /></label>
            <label class="campo"
              >Valor (R$)<input v-model.number="novaContaValor" type="number" min="0" step="0.01"
            /></label>
            <label class="campo">Vencimento<input v-model="novaContaVenc" type="date" /></label>
            <label class="campo"
              >Tipo
              <select v-model="novaContaTipo">
          ...
          <button type="button" class="botao-primario" :disabled="!podeLancar" @click="adicionarConta">
            Adicionar
          </button>
        </div>
      ...
      <template v-if="aba === 'detalhe'">
        <div class="cartoes">
          <div class="cartao">
            <span>Receita</span><strong>{{ formatarMoeda(resumo.receita) }}</strong>
          </div>
          <div class="cartao">
            <span>Lucro líquido</span><strong>{{ formatarMoeda(resumo.lucroLiquido) }}</strong>
          </div>
          <div class="cartao">
            <span>Margem líquida</span
            ><strong>{{
          ...
          <div class="cartao">
            <span>CMV</span><strong>{{ formatarMoeda(resumo.cmv) }}</strong>
          </div>
          <div class="cartao">
            <span>Perdas</span><strong>{{ formatarMoeda(resumo.desperdicio) }}</strong>
          </div>
          <div class="cartao">
            <span>Ponto de equilíbrio/mês</span
            ><strong>{{ pe === null ? 'sem base' : formatarMoeda(pe) }}</strong>
          </div>
        </div>
        ...
        <div v-if="comparativo" class="cartao-secao">
          ...
          <div class="grade-dupla">
            <div class="linha-custo">
              <span>Receita atual × anterior</span
              ><strong
                >{{ formatarMoeda(comparativo.receitaAtual) }} ×
                {{ formatarMoeda(comparativo.receitaAnterior)
                }}{{
                  comparativo.variacaoReceita === null
                    ? ''
                    : ` (${(comparativo.variacaoReceita * 100).toFixed(1)}%)`
                }}</strong
              >
            </div>
            <div class="linha-custo">
              <span>Lucro atual × anterior</span
              ><strong
                >{{ formatarMoeda(comparativo.lucroAtual) }} × {{ formatarMoeda(comparativo.lucroAnterior)
                }}{{
                  comparativo.variacaoLucro === null
                    ? ''
                    : ` (${(comparativo.variacaoLucro * 100).toFixed(1)}%)`
                }}</strong
              >
            </div>
          </div>
        ...
        <div class="cartao-secao">
          <div class="secao-cabecalho">
            ...
            <button type="button" class="botao-exportar" @click="exportarDetalhado">
              Exportar detalhado
            </button>
          </div>
          <GraficoLinha :rotulos="serie.rotulos" :series="linhasResultado" />
          <div class="legenda">
            Lucro = receita − CMV − despesas. Acima de 31 dias os valores são somados por semana.
          </div>
        </div>
        ...
        <div class="grade-dupla">
          ...
          <div class="cartao-secao">
            ...
            <div class="legenda">
              Saldo acumulado:
              <strong>{{ formatarMoeda(fluxoAcumulado[fluxoAcumulado.length - 1] ?? 0) }}</strong> · Projeção
              30d: <strong>{{ projecaoCaixa === null ? 'sem base' : formatarMoeda(projecaoCaixa) }}</strong> ·
              Prazo médio de recebimento (estimado):
              <strong>{{ pmr === null ? 'sem base' : `${pmr} dias` }}</strong>
            </div>
          </div>
        </div>
        ...
        <div v-if="podeVerMargem" class="cartao-secao">
          ...
          <div v-for="item in margemTop" :key="item.produtoId" class="barra-h">
            ...
            <span class="barra-h-trilho"
              ><span
                class="barra-h-preenchimento"
                :style="{ width: Math.round((item.margem / maxMargemTop) * 100) + '%' }"
            /></span>
        ...
        <div class="cartao-secao">
          ...
          <div v-for="item in resumoDesp.porMotivo" :key="item.motivo" class="barra-h">
            ...
            <span class="barra-h-trilho"
              ><span
                class="barra-h-preenchimento perda"
                :style="{ width: Math.round((item.valor / Math.max(1, resumoDesp.totalReais)) * 100) + '%' }"
            /></span>
            <strong>{{ formatarMoeda(item.valor) }}</strong>
          </div>
        </div>
        ...
        <div class="grade-dupla">
          ...
          <div class="cartao-secao">
            ...
            <div v-if="rendimento.ordens === 0" class="legenda">
              Nenhuma desossa concluída — veja Desossa &amp; Subprodutos.
            </div>
            <template v-else>
              <div class="linha-custo">
                <span>Ordens</span><strong>{{ rendimento.ordens }}</strong>
              </div>
              <div class="linha-custo">
                <span>Recebido × aproveitável</span
                ><strong>{{ rendimento.recebidoKg }} kg × {{ rendimento.aproveitavelKg }} kg</strong>
              </div>
              <div class="linha-custo">
                <span>Rendimento</span
                ><strong>{{
                  rendimento.rendimento === null ? '—' : formatarPercentual(rendimento.rendimento)
                }}</strong>
              </div>
              <div class="linha-custo">
                <span>Custo efetivo médio</span
                ><strong>{{
                  rendimento.custoEfetivoMedio === null
                    ? '—'
                    : `${formatarMoeda(rendimento.custoEfetivoMedio)}/kg`
                }}</strong>
              </div>
            </template>
          </div>
        ...
        <div class="cartao-secao">
          ...
          <div class="legenda">
            Valor = quantidade em estoque × custo médio. Capital parado em itens de baixo giro aparece no topo
      ...
      <template v-if="aba === 'relatorios'">
        <div class="cartao-secao">
          <div class="secao-cabecalho">
            ...
            <button type="button" class="botao-exportar" @click="exportarRelatorios">
              Exportar relatórios
            </button>
          </div>
          <div class="grade-params">
          ...
          <div class="legenda">
            Grupo/subgrupo vêm do snapshot da venda — mudança no catálogo não reescreve o histórico.
            <button type="button" class="link" @click="limparFiltrosRel">limpar filtros</button>
          </div>
          <div v-if="mercado.vendas.length === 0" class="legenda">
            Sem vendas na sessão —
            <button type="button" class="link" @click="carregarDemoFin">carregar demonstração</button> para
            visualizar.
          </div>
        </div>
        ...
        <div class="cartoes">
          <div class="cartao">
            <span>Auditoria · válidas</span><strong>{{ resumoAud.vendasValidas }}</strong>
          </div>
          <div class="cartao">
            <span>Canceladas</span><strong>{{ resumoAud.canceladas }}</strong>
          </div>
          <div class="cartao">
            <span>Devoluções</span><strong>{{ resumoAud.devolvidas }}</strong>
          </div>
          <div class="cartao">
            <span>Descontos</span><strong>{{ formatarMoeda(resumoAud.descontoTotal) }}</strong>
          </div>
          <div class="cartao">
            <span>Reimpressões</span><strong>{{ resumoAud.reimpressoes }}</strong>
          </div>
        </div>
          ...
          <table v-else class="tabela">
            ...
            <tbody>
              <tr v-for="l in relGrupo" :key="`${l.grupo}|${l.subgrupo}`">
                <td>
                  {{ l.grupo }} · {{ l.subgrupo }}<br /><small
                    >{{ l.vendas }} vendas · {{ l.quantidade }} qtd · desc.
                    {{ formatarMoeda(l.desconto) }}</small
                  >
                </td>
          ...
          <table v-else class="tabela">
            ...
            <tbody>
              <tr v-for="m in relMargemReal.slice(0, 12)" :key="m.produtoId">
                <td>
                  {{ m.produto }}<br /><small
                    >{{ m.grupo }} · {{ m.subgrupo }} · {{ m.quantidade }} qtd</small
                  >
                </td>
        ...
        <div class="cartao-secao">
          ...
          <div class="mapa-hora" role="img" aria-label="Mapa de calor por hora">
            <div
              ...
              :key="h.hora"
              class="celula-hora"
              :class="{ ativa: h.vendas > 0 }"
              :style="{ opacity: h.vendas > 0 ? 0.35 + 0.65 * (h.receita / maxHoraReceita) : 1 }"
              :title="`${String(h.hora).padStart(2, '0')}h: ${h.vendas} vendas · ${formatarMoeda(h.receita)}`"
            >
          ...
          <div class="legenda">
            Cancelada não soma receita — aparece na coluna de cancelamentos e na auditoria.
          </div>
        </div>
            ...
            <table v-else class="tabela">
              ...
              <tbody>
                <tr v-for="o in relOperador" :key="o.operador">
                  <td>
                    {{ o.operador }}<br /><small
                      >ticket {{ o.ticketMedio === null ? '—' : formatarMoeda(o.ticketMedio) }} · canc.
                      {{ o.cancelamentos }}</small
                    >
                  </td>
                  <td>{{ o.vendas }}</td>
          ...
          <div class="cartao-secao">
            ...
            <div v-for="p in relPagamento" :key="p.forma" class="barra-h">
              ...
              <span class="barra-h-trilho"
                ><span
                  class="barra-h-preenchimento"
                  :style="{ width: Math.round(p.participacao * 100) + '%' }"
              /></span>
            ...
            <table v-else class="tabela">
              ...
              <tbody>
                <tr v-for="f in relFornecedor" :key="f.fornecedor">
                  <td>
                    {{ f.fornecedor }}<br /><small>{{ f.itens }} itens</small>
                  </td>
