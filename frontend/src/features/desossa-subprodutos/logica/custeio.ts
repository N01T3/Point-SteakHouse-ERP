...
export function calcularPesoAproveitavelKg(
  saidas: { classificacao: ClassificacaoDeSaida; pesoKg: number }[],
): number {
  return saidas
    .filter((saida) => saida.classificacao === 'CORTE')
    .reduce((soma, saida) => soma + saida.pesoKg, 0)
}
...
export function desvioDeRendimentoSignificativo(
  rendimentoRealizado: number,
  rendimentoEsperado: number,
): boolean {
  return (
    Math.abs(rendimentoRealizado - rendimentoEsperado) >
    LIMIAR_DE_DESVIO_DE_RENDIMENTO + EPSILON_DE_PONTO_FLUTUANTE
  )
}
