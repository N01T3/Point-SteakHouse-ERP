...
describe('desossa completa', () => {
  it('cadastra peça, valida soma e impede envio duplicado', () => {
    ...
    const peca = s.registrarPecaBruta({
      fornecedor: 'F',
      tipoDePeca: 'Traseiro',
      pesoKg: 100,
      custoPorKg: 20,
    ...
    expect(() =>
      s.concluirDesossa(
        peca.id,
        [{ id: 'x', nome: 'Picanha', pesoKg: 120, classificacao: 'CORTE', destino: null }],
        0.5,
      ),
    ).toThrow(/ultrapassa/)
  ...
  it('cancela ordem não enviada e devolve peça', () => {
    ...
    const ordem = s.concluirDesossa(
      peca.id,
      [{ id: 'a', nome: 'Acém', pesoKg: 9, classificacao: 'CORTE', destino: null }],
      0.3,
    )
...
describe('maturação iniciada pelo estoque', () => {
  it('inicia, pesa e impede finalização duplicada', () => {
    ...
    const p = m.iniciarMaturacao({
      nome: 'Ancho',
      tecnica: 'dry_aged',
      camaraId: 'camara-1',
      pesoInicialKg: 5,
      diasTotal: 30,
      custoInicialPorKg: 50,
      loteOrigem: 'L1',
    })
