...
describe('fonte única operacional', () => {
  it('venda gera livro + resumo operacional', () => {
    ...
    turno(store)
    const produto = store.produtos.find((p) =>
      p.lotes.some((l) => l.estado === 'LIBERADO' && l.quantidade > 1),
    )!
  ...
  it('desossa e maturação viram estoque com rastreabilidade', () => {
    ...
    const total = store.aplicarDesossaAoEstoque(
      {
        pecaBruta: { id: 'peca-1', fornecedor: 'F' },
        saidas: [{ nome: 'Corte Teste', pesoKg: 5, classificacao: 'CORTE', destino: null }],
        custoEfetivoPorKg: 40,
      },
      'dono',
    )
    expect(total).toBe(1)
    ...
    store.aplicarMaturacaoAoEstoque(
      { id: 'peca-m1', nome: 'Ancho Teste', pesoAtualKg: 8, custoInicialPorKg: 50, pesoInicialKg: 10 },
      'dono',
    )
