    ...
    const cliente = store.cadastrarCliente(
      {
        ...
        aniversario: fAniversario.value || undefined,
        limite: fLimite.value,
      },
  ...
  <div class="pagina">
    <div class="cabecalho">
      <div>
        ...
        <div class="subtitulo">
          {{ store.resumo.devedores }} com saldo em aberto · {{ formatarMoeda(store.resumo.totalAberto) }}
        </div>
      </div>
      <div class="abas">
        <button type="button" :class="{ ativa: aba === 'clientes' }" @click="aba = 'clientes'">
          Clientes
        </button>
        <button type="button" :class="{ ativa: aba === 'cobranca' }" @click="aba = 'cobranca'">
          Cobrança
        </button>
      </div>
    </div>
    ...
    <template v-if="aba === 'clientes'">
      ...
      <div v-if="mostrandoCadastro && podeGerenciar" class="cartao">
        ...
        <div class="grade">
          ...
          <label class="campo"
            >Telefone<input v-model="fTelefone" type="tel" placeholder="(44) 99999-9999"
          /></label>
          ...
          <label class="campo"
            >Limite (R$)*<input v-model.number="fLimite" type="number" min="0" step="0.01"
          /></label>
        </div>
      ...
      <div class="lista">
        <button v-for="c in lista" :key="c.id" type="button" class="item" @click="detalhe = c">
          ...
          <div class="item-meta">
            {{ c.telefone ?? 'sem telefone' }}{{ ehAniversario(c) ? ' · 🎂 hoje!' : '' }}
          </div>
    ...
    <template v-if="aba === 'cobranca'">
      <div class="cartoes">
        <div class="cartao-mini">
          <span>Total em aberto</span><strong>{{ formatarMoeda(store.resumo.totalAberto) }}</strong>
        </div>
        <div class="cartao-mini">
          <span>Devedores</span><strong>{{ store.resumo.devedores }}</strong>
        </div>
        <div class="cartao-mini">
          <span>Bloqueados</span><strong>{{ store.resumo.bloqueados }}</strong>
        </div>
      </div>
        ...
        <div class="grade">
          ...
          <label class="campo"
            ...
            <span class="campo-valor">
              ...
              <button type="button" class="botao-secundario" :disabled="!clienteQuitar" @click="quitarTotal">
                Total
              </button>
            </span>
          </label>
          <label class="campo"
            >Observação<input v-model="qObs" type="text" placeholder="dinheiro, PIX…"
          /></label>
        </div>
        <div v-if="clienteQuitar" class="nota">
          Disponível após pagar: {{ formatarMoeda(clienteQuitar.limite) }} · em aberto:
          {{ formatarMoeda(clienteQuitar.saldo) }}
        </div>
    ...
    <div v-if="detalhe" class="fundo" @click.self="detalhe = null">
      <div class="modal" role="dialog" aria-label="Conta do cliente">
        ...
        <div class="modal-sub">
          {{ detalhe.telefone ?? '—' }}{{ detalhe.endereco ? ` · ${detalhe.endereco}` : '' }}
        </div>

        <div class="bloco">
          <div class="linha">
            <span>Saldo em aberto</span><strong>{{ formatarMoeda(detalhe.saldo) }}</strong>
          </div>
          <div class="linha">
            <span>Limite</span><strong>{{ formatarMoeda(detalhe.limite) }}</strong>
          </div>
          <div class="linha">
            <span>Disponível</span><strong>{{ formatarMoeda(fiadoDisponivel(detalhe)) }}</strong>
          </div>
          <div class="linha">
            <span>Status</span><strong>{{ contaBloqueada(detalhe) ? 'bloqueado' : 'liberado' }}</strong>
          </div>
        </div>
        ...
        <div v-if="podeGerenciar" class="acoes-detalhe">
          ...
          <button
            ...
            @click="store.alternarAtivo(detalhe.id); detalhe = store.clientes.find((c) => c.id === detalhe?.id) ?? null"
          >
          ...
          <button
            type="button"
            class="botao-secundario"
            @click="aba = 'cobranca'; qClienteId = detalhe.id; qValor = detalhe.saldo; detalhe = null"
          >
            Quitar →
          </button>
        </div>
        ...
        <div v-for="l in store.extratoDoCliente(detalhe.id)" :key="l.id" class="linha">
          <span
            >{{ rotuloLancamento(l.tipo, l.vendaNumero) }} · {{ formatarData(l.criadoEm)
            }}{{ l.observacao ? ` · ${l.observacao}` : '' }}</span
          >
          <strong :class="l.tipo"
            >{{ l.tipo === 'pagamento' ? '−' : '+' }}{{ formatarMoeda(l.valor) }}</strong
          >
        </div>
        ...
        <div class="nota">
          {{
            detalhe.cortesPreferidos?.length
              ? detalhe.cortesPreferidos.join(' · ')
              : 'Sem cortes preferidos registrados.'
          }}{{ detalhe.observacoes ? ` — ${detalhe.observacoes}` : '' }}
        </div>
        <div v-if="podeGerenciar" class="grade">
          <label class="campo"
            >Cortes preferidos (vírgula)<input
              v-model="prefCortes"
              type="text"
              placeholder="ex.: picanha, ancho"
          /></label>
          <label class="campo"
            >Observações<input v-model="prefObs" type="text" placeholder="ex.: prefere dry aged"
          /></label>
        </div>
        <button v-if="podeGerenciar" type="button" class="botao-secundario" @click="salvarPrefs">
          Salvar preferências
        </button>

        <div class="secao-titulo">
          Reservas e encomendas ({{
            store.reservas.filter((r) => r.clienteId === (detalhe?.id ?? '')).length
          }})
        </div>
        <div
          v-for="r in store.reservas.filter((x) => x.clienteId === (detalhe?.id ?? ''))"
          :key="r.id"
          class="linha"
        >
        ...
        <div v-if="podeGerenciar" class="grade">
          <label class="campo"
            >Nova reserva<input v-model="resDesc" type="text" placeholder="ex.: Picanha maturada 2kg"
          /></label>
          <label class="campo">Qtd<input v-model="resQtd" type="text" placeholder="ex.: 2 kg" /></label>
        </div>
        <button v-if="podeGerenciar" type="button" class="botao-secundario" @click="criarRes">
          Reservar
        </button>
