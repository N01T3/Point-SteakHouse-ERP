...
const itensPedido = ref<Array<{ produtoId: string; produto: string; quantidade: number; custoUnitario: number }>>([])
...
function confirmarTransferencia(): void {
  try {
    ...
    const tr = store.transferirEstoque(trProdutoId.value, trLote.value, trOrigem.value, trDestino.value, trQtd.value, trMotivo.value, operador.value)
    trQtd.value = null
    trMotivo.value = ''
...
function confirmarContagem(): void {
  try {
    ...
    const cg = store.registrarContagem(invProdutoId.value, invLote.value, invLocal.value, invContado.value, invMotivo.value, operador.value)
    invContado.value = null
    ok(cg.diferenca === 0 ? 'Contagem confere com o sistema.' : `Divergência de ${cg.diferenca} registrada — aguarda aprovação.`)
  } catch (e) {
    falha(e)
  }
}
...
function adicionarItemPedido(): void {
  ...
  itensPedido.value.push({ produtoId: produto.id, produto: produto.nome, quantidade: pedQtd.value, custoUnitario: pedCusto.value ?? produto.custoMedio })
  pedProdutoId.value = ''
  pedQtd.value = null
  pedCusto.value = null
}
...
<template>
  <div class="pagina">
    ...
    <div class="subabas">
      ...
      <button type="button" :class="{ ativa: subAba === 'transferir' }" @click="subAba = 'transferir'">Transferir</button>
      <button type="button" :class="{ ativa: subAba === 'inventario' }" @click="subAba = 'inventario'">
        Inventário{{ store.contagens.filter((c) => c.status === 'pendente').length ? ` (${store.contagens.filter((c) => c.status === 'pendente').length})` : '' }}
      </button>
      <button type="button" :class="{ ativa: subAba === 'fornecedores' }" @click="subAba = 'fornecedores'">Fornecedores</button>
    ...
    <div v-if="subAba === 'visao'" class="visao">
      <div class="resumo-grade">
        <div class="resumo-card"><span>SKUs</span><strong>{{ resumoVisao.skus }}</strong></div>
        <div class="resumo-card"><span>Carnes (kg)</span><strong>{{ resumoVisao.kg.toFixed(1) }} kg</strong></div>
        <div class="resumo-card"><span>Unidades</span><strong>{{ resumoVisao.un }}</strong></div>
        <div class="resumo-card" :class="{ alerta: resumoVisao.criticos > 0 }"><span>Críticos / zerados</span><strong>{{ resumoVisao.criticos }}</strong></div>
        <div class="resumo-card" :class="{ alerta: resumoVisao.vencendo > 0 }"><span>Lotes vencendo</span><strong>{{ resumoVisao.vencendo }}</strong></div>
      </div>

      <div class="filtros-visao">
        <input v-model="buscaVisao" type="text" placeholder="Buscar produto…" class="input-busca" aria-label="Buscar produto no estoque" />
        <div class="segmentado" role="group" aria-label="Filtrar por situação">
          <button type="button" :class="{ ativo: filtroVisao === 'todos' }" @click="filtroVisao = 'todos'">Todos</button>
          <button type="button" :class="{ ativo: filtroVisao === 'criticos' }" @click="filtroVisao = 'criticos'">Críticos</button>
          <button type="button" :class="{ ativo: filtroVisao === 'zerados' }" @click="filtroVisao = 'zerados'">Zerados</button>
          <button type="button" :class="{ ativo: filtroVisao === 'vencendo' }" @click="filtroVisao = 'vencendo'">Vencendo</button>
        </div>
      </div>

      <div v-if="produtosVisao.length === 0" class="cartao vazio">Nenhum produto neste filtro — ajuste a busca.</div>

      <div class="grade-produtos">
        <article v-for="{ produto: p, total } in produtosVisao" :key="p.id" class="card-produto" :class="statusDoProduto(p.id, total, p.estoqueMinimo)">
          <div class="card-topo">
            <div class="card-titulos">
              <strong class="card-nome">{{ p.nome }}</strong>
              <span class="card-meta">{{ p.grupo ?? p.categoria }}{{ p.subgrupo ? ` · ${p.subgrupo}` : '' }}</span>
              <span class="card-meta secundario">{{ p.fornecedor ?? 'Sem fornecedor' }} · {{ p.unidade === 'KG' ? 'venda por peso' : 'venda por unidade' }}</span>
            </div>
            <span class="badge-status" :class="statusDoProduto(p.id, total, p.estoqueMinimo)">{{ ROTULO_STATUS_PRODUTO[statusDoProduto(p.id, total, p.estoqueMinimo)] }}</span>
          </div>
          ...
          <div class="barra-estoque" role="img" :aria-label="`Estoque ${progressoEstoque(total, p.estoqueAlvo)}% do alvo`">
            <span class="barra-preenchimento" :style="{ width: progressoEstoque(total, p.estoqueAlvo) + '%' }" />
          </div>

          <div v-if="p.lotes.length === 0" class="nota">Sem lotes — receba no Mercado → Administração → Recebimento.</div>
          <ul v-else class="lotes">
            <li v-for="l in p.lotes" :key="`${l.lote}-${localDoLote(l)}`" class="lote-linha">
              ...
              <div class="lote-info">
                <span class="lote-nome"><code>{{ l.lote }}</code> · {{ localDoLote(l) }}</span>
                <span class="lote-meta">
                  <span class="badge-lote" :class="classeEstado(l.estado)">{{ l.estado.toLowerCase() }}</span>
                  <span class="validade" :class="classeValidade(l.validade)">{{ rotuloValidade(l.validade) }}</span>
                </span>
              </div>
              <strong class="lote-qtd">{{ l.quantidade }}<small>{{ p.unidade === 'KG' ? ' kg' : ' un' }}</small></strong>
            </li>
          </ul>
        </article>
      </div>
    ...
    <div v-if="subAba === 'transferir'" class="cartao">
      ...
      <div class="grade">
        ...
        <label class="campo">Origem
          <select v-model="trOrigem"><option v-for="loc in LOCAIS_ESTOQUE" :key="loc" :value="loc">{{ loc }}</option></select>
        </label>
        <label class="campo">Destino
          <select v-model="trDestino"><option v-for="loc in LOCAIS_ESTOQUE" :key="loc" :value="loc">{{ loc }}</option></select>
        </label>
        <label class="campo">Qtd<input v-model.number="trQtd" type="number" min="0" step="0.001" /></label>
        <label class="campo">Motivo<input v-model="trMotivo" type="text" placeholder="ex.: abastecer balcão" /></label>
      </div>
      ...
      <div v-for="tr in store.transferencias" :key="tr.id" class="linha-simples">
        <span>{{ tr.produto }} · lote {{ tr.lote }} · {{ tr.origem }} → {{ tr.destino }} · {{ tr.motivo }}</span>
    ...
    <div v-if="subAba === 'inventario'" class="cartao">
      ...
      <div class="grade">
        ...
        <label class="campo">Local
          <select v-model="invLocal"><option v-for="loc in LOCAIS_ESTOQUE" :key="loc" :value="loc">{{ loc }}</option></select>
        </label>
        <label class="campo">Contado<input v-model.number="invContado" type="number" min="0" step="0.001" /></label>
        <label class="campo">Motivo<input v-model="invMotivo" type="text" placeholder="contagem cíclica" /></label>
      </div>
      ...
      <div v-for="cg in store.contagens" :key="cg.id" class="linha-simples">
        <span>{{ cg.produto }} · lote {{ cg.lote }} · {{ cg.local }} · sistema {{ cg.sistema }} × contado {{ cg.contado }} · {{ cg.status }}</span>
        <span class="acoes-linha">
          <strong>{{ cg.diferenca > 0 ? '+' : '' }}{{ cg.diferenca }}</strong>
          <button v-if="cg.status === 'pendente'" type="button" class="link" :disabled="!podeAjustar" @click="aprovar(cg.id)">aprovar</button>
        </span>
      </div>
    ...
    <div v-if="subAba === 'fornecedores'" class="cartao">
      ...
      <div v-for="f in store.fornecedores" :key="f.id" class="linha-simples">
        <span>{{ f.nome }}{{ f.contato ? ` · ${f.contato}` : '' }}{{ f.leadTimeDias ? ` · entrega em ${f.leadTimeDias}d` : '' }}</span>
      </div>
      ...
      <div class="grade">
        ...
        <label class="campo">Custo unit. (R$)<input v-model.number="pedCusto" type="number" min="0" step="0.01" /></label>
      </div>
      <div class="botoes-linha">
        ...
        <button type="button" class="botao-primario" :disabled="itensPedido.length === 0" @click="fecharPedido">Abrir pedido</button>
      </div>
      ...
      <div v-for="ped in store.pedidos" :key="ped.id" class="linha-simples">
        <span>#{{ ped.numero }} · {{ ped.fornecedor }} · {{ ped.itens.length }} itens · {{ ROTULO_STATUS[ped.status] }}</span>
        <span class="acoes-linha">
          <strong>{{ formatarMoeda(pedidoTotal(ped)) }}</strong>
          <select :value="ped.status" aria-label="Status do pedido" @change="trocarStatus(ped.id, ($event.target as HTMLSelectElement).value)">
    ...
    <div v-if="subAba === 'recall'" class="cartao">
      ...
      <template v-if="recallResultado">
        <div class="linha-simples"><span>Produto</span><strong>{{ recallResultado.produto ?? '—' }}</strong></div>
        <div v-if="recallResultado.estoquePorLocal.length === 0" class="nota">Sem estoque atual deste lote.</div>
        <div v-for="(e, i) in recallResultado.estoquePorLocal" :key="i" class="linha-simples">
          <span>{{ e.local }} · {{ e.estado }}</span><strong>{{ e.quantidade }}</strong>
        </div>
        ...
        <div v-for="v in recallResultado.vendas" :key="v.numero" class="linha-simples">
          <span>venda #{{ v.numero }} · {{ dataCurta(v.criadaEm) }} · {{ v.operador }}{{ v.clienteNome ? ` · ${v.clienteNome}` : '' }}</span>
          <strong>{{ v.quantidade }}</strong>
        </div>
        <div v-if="'movimentos' in recallResultado && (recallResultado as { movimentos?: Array<{ id: string; tipo: string; motivo: string; quantidade: number }> }).movimentos?.length" class="secao-titulo espacar">Cadeia do lote (livro)</div>
        <div v-for="m in ((recallResultado as { movimentos?: Array<{ id: string; tipo: string; motivo: string; quantidade: number }> }).movimentos ?? [])" :key="m.id" class="linha-simples">
          <span>{{ m.tipo }} · {{ m.motivo }}</span><strong>{{ m.quantidade }}</strong>
        </div>
      </template>
    </div>
    <!-- QUARENTENA -->
    <div v-if="subAba === 'visao'" class="cartao">
      ...
      <template v-for="p in store.produtos" :key="p.id">
        <div v-for="l in p.lotes.filter((x) => x.estado === 'QUARENTENA')" :key="`${p.id}-${l.lote}`" class="linha-simples">
          <span>{{ p.nome }} · lote {{ l.lote }} · {{ l.motivoBloqueio ?? 'em conferência' }}</span>
          <button type="button" class="link" :disabled="!podeAjustar" @click="liberarLote(p.id, l.lote)">liberar</button>
        </div>
      </template>
    </div>
  </div>
...
.bloco { display: flex; flex-direction: column; gap: 2px; padding: 8px 0; border-top: 1px solid var(--cor-outline); }
...
.botao-secundario { padding: 11px 20px; border-radius: 100px; border: 1px solid var(--cor-outline); background: var(--cor-bg); color: var(--cor-on-surface); font-family: inherit; font-size: 13px; font-weight: 600; cursor: pointer; }

/* --- Visão: resumo + cards por produto --- */
.visao { display: flex; flex-direction: column; gap: 14px; }
.resumo-grade { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; }
.resumo-card { background: var(--cor-surface); border: 1px solid var(--cor-outline); border-radius: 14px; padding: 14px 16px; display: flex; flex-direction: column; gap: 4px; }
.resumo-card span { font-size: 12px; color: var(--cor-on-surface-variant); }
.resumo-card strong { font-family: 'Bodoni Moda', serif; font-size: 20px; color: var(--cor-on-surface); }
.resumo-card.alerta { border-color: var(--cor-error); }
.resumo-card.alerta strong { color: var(--cor-error); }
.filtros-visao { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
...
.segmentado { display: inline-flex; border: 1px solid var(--cor-outline); border-radius: 100px; overflow: hidden; background: var(--cor-surface); }
.segmentado button { border: none; background: transparent; padding: 10px 14px; min-height: 44px; font-family: inherit; font-size: 12.5px; font-weight: 600; color: var(--cor-on-surface-variant); cursor: pointer; white-space: nowrap; }
.segmentado button.ativo { background: var(--cor-primary-container); color: var(--cor-on-primary-container); }
.grade-produtos { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 12px; }
.card-produto { background: var(--cor-surface); border: 1px solid var(--cor-outline); border-left: 4px solid var(--cor-outline); border-radius: 16px; padding: 16px 18px; display: flex; flex-direction: column; gap: 12px; }
.card-produto.ok { border-left-color: var(--cor-success); }
.card-produto.critico { border-left-color: var(--cor-error); }
.card-produto.zerado { border-left-color: var(--cor-error); background: color-mix(in srgb, var(--cor-error-container) 35%, var(--cor-surface)); }
.card-topo { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; }
.card-titulos { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.card-nome { font-size: 15px; font-weight: 700; color: var(--cor-on-surface); }
.card-meta { font-size: 12px; color: var(--cor-on-surface-variant); }
.card-meta.secundario { font-size: 11.5px; opacity: 0.9; }
.badge-status { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; padding: 4px 10px; border-radius: 100px; flex-shrink: 0; }
.badge-status.ok { color: var(--cor-success); background: var(--cor-surface-variant); }
.badge-status.critico { color: var(--cor-error); background: var(--cor-error-container); }
.badge-status.zerado { color: #fff; background: var(--cor-error); }
.card-total { display: flex; align-items: baseline; justify-content: space-between; gap: 10px; }
.card-total strong { font-family: 'Bodoni Moda', serif; font-size: 24px; color: var(--cor-on-surface); }
.card-total span { font-size: 12px; color: var(--cor-on-surface-variant); }
.barra-estoque { height: 6px; border-radius: 100px; background: var(--cor-surface-variant); overflow: hidden; }
.barra-preenchimento { display: block; height: 100%; background: var(--cor-primary); border-radius: 100px; }
.card-produto.critico .barra-preenchimento, .card-produto.zerado .barra-preenchimento { background: var(--cor-error); }
.lotes { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; }
.lote-linha { display: flex; align-items: center; gap: 10px; padding: 10px 0; border-top: 1px solid var(--cor-outline); }
.lote-ponto { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; background: var(--cor-outline); }
.lote-ponto.liberado { background: var(--cor-success); }
.lote-ponto.quarentena { background: var(--cor-secondary); }
.lote-ponto.bloqueado { background: var(--cor-error); }
.lote-ponto.outro { background: var(--cor-on-surface-variant); }
.lote-info { display: flex; flex-direction: column; gap: 4px; flex: 1; min-width: 0; }
.lote-nome { font-size: 13px; color: var(--cor-on-surface); }
.lote-nome code { font-family: ui-monospace, monospace; font-size: 12.5px; background: var(--cor-surface-variant); padding: 1px 6px; border-radius: 6px; }
.lote-meta { display: flex; gap: 6px; align-items: center; flex-wrap: wrap; font-size: 11.5px; color: var(--cor-on-surface-variant); }
.badge-lote { font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; padding: 2px 8px; border-radius: 100px; background: var(--cor-surface-variant); color: var(--cor-on-surface-variant); }
.badge-lote.liberado { color: var(--cor-success); }
.badge-lote.quarentena { color: var(--cor-secondary); }
.badge-lote.bloqueado { color: var(--cor-error); background: var(--cor-error-container); }
.validade.ok { color: var(--cor-on-surface-variant); }
.validade.atencao { color: var(--cor-secondary); font-weight: 600; }
.validade.critica { color: var(--cor-error); font-weight: 700; }
.validade.vencido { color: #fff; background: var(--cor-error); padding: 1px 8px; border-radius: 100px; font-weight: 700; }
.lote-qtd { font-size: 14px; color: var(--cor-on-surface); white-space: nowrap; }
.lote-qtd small { font-size: 11px; color: var(--cor-on-surface-variant); font-weight: 400; }
.cartao.vazio { align-items: center; text-align: center; }
@media (max-width: 640px) { .grade-produtos { grid-template-columns: 1fr; } }
</style>
